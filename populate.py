...
def insert_manufacturer(cursor, name):
    cursor.execute(
        "insert into Manufacturers (name) values (%s) returning id", (name, ))
    created_id = cursor.fetchone()[0]
    return created_id
...
def insert_machine(cursor, model):
    cursor.execute(
        "insert into Machines (modelNumber) values (%s) returning serialNumber", (model, ))
    created_id = cursor.fetchone()[0]
    return created_id
...
def insert_order(cursor, customer, machine):
    cursor.execute(
        "insert into Orders (customerId, serialNumber) values (%s, %s) returning number", (customer, machine))
    created_id = cursor.fetchone()[0]
    return created_id


def insert_model(cursor, number, speed, ram, hd, price, manufacturerId):
    cursor.execute("insert into Models (id, speed, ram, hd, price, manufacturerId) values (%s, %s, %s, %s, %s, %s) returning id",
                   (number, speed, ram, hd, price, manufacturerId))
    created_id = cursor.fetchone()[0]
    return created_id
...
with psycopg2.connect(database=url.path[1:], user=url.username, password=url.password, host=url.hostname, port=url.port) as conn:
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            'create table Customers (id serial NOT NULL PRIMARY KEY, name text)')
        cursor.execute(
            'create table Machines (serialNumber serial NOT NULL PRIMARY KEY, modelNumber int)')
        cursor.execute(
            'create table Models (id int NOT NULL PRIMARY KEY, speed real, ram int, hd int, price real, manufacturerId int)')
        cursor.execute(
            'create table Manufacturers (id serial NOT NULL PRIMARY KEY, name text)')
